...
class LeaveAppliedNotofication extends Notification
{
    use Queueable;
     ...
     */
    public function __construct(public $leave)
    {
        $this->leave = $leave;
    }
    ...
    {
        return [
            'message' => match ($this->leave->status) {
                'pending' => ($this->leave->user?->full_name ?? 'An employee') . ' applied for leave.',
